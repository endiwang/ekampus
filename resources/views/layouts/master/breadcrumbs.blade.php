        ...
        @if (isset($buttons) && is_array($buttons) && count($buttons))
            <div class="d-flex align-items-center gap-2 gap-lg-3">
                @foreach ($buttons as $button)
                    @if (isset($button['is_show']) && $button['is_show'] == false)
                        @continue
                    @endif
            ...
            <div class="d-flex align-items-center gap-2 gap-lg-3">
                @foreach ($modals as $modal)
                    <a href="#" data-bs-toggle="modal" data-bs-target="{{ $modal['id'] }}"
                        class="{{ $modal['button_class'] }}" title="{{ $modal['title'] }}">
                        <i class="{{ $modal['icon_class'] }}"
                            style="vertical-align: initial"></i>{{ $modal['title'] }}
                    </a>
                @endforeach
            </div>
        @endif
